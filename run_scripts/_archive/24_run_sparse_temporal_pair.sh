#!/usr/bin/env bash
# Warm-start alternating frame-wise/causal-global attention from Sparse-VGGT.
set -euo pipefail
...
EVAL_ROOT="outputs/eval_sparse_temporal_pair"
...
"$PYTHON" -u -m scripts.smoke_test_sparse_temporal \
  --config configs/wcm_sparse_temporal_exp_s3072.yaml --device cuda --batch-size 1

for SEED in 3072 42; do
  CFG="configs/wcm_sparse_temporal_exp_s${SEED}.yaml"
  OUT="$BASE/wcm_sparse_temporal_exp_s${SEED}"
  ...
  "$PYTHON" -u -m world_critic.train --config "$CFG" \
    2>&1 | tee "outputs/batch_logs/wcm_sparse_temporal_exp_s${SEED}.log"
done
...
run_eval() {
  ...
  "$PYTHON" -u -m world_critic.evaluate \
    ...
    > "outputs/batch_logs/temporal_${dataset_name}_${model_name}.log" 2>&1
}
...
for DATASET_NAME in 5cut ood; do
  ...
  for SEED in 3072 42; do
    ...
    run_eval "$DATASET_NAME" "$DATASET_ROOT" "temporal_s$SEED" \
      "$BASE/wcm_sparse_temporal_exp_s$SEED/deploy.pt"
    "$PYTHON" -m scripts.compare_experiments \
      "$EVAL_ROOT/$DATASET_NAME/image_b4_s$SEED" \
      "$EVAL_ROOT/$DATASET_NAME/temporal_s$SEED" \
      --align common --seed "$SEED" \
      --output "$EVAL_ROOT/comparisons/${DATASET_NAME}_temporal_vs_image_b4_s${SEED}.json"
  done
done
