#!/usr/bin/env bash
# Train persistent sparse causal memory from scratch, then compare to Image-B4.
set -euo pipefail
...
EVAL_ROOT="outputs/eval_sparse_memory_scratch_pair"
mkdir -p outputs/batch_logs "$EVAL_ROOT/comparisons"

echo "== 0/4 unit tests and CUDA memory/gradient gate =="
"$PYTHON" -m pytest tests/test_register_config.py tests/test_temporal_input.py -q
"$PYTHON" -u -m scripts.smoke_test_sparse_memory \
  --config configs/wcm_sparse_memory_scratch_s3072.yaml --device cuda --batch-size 4

echo "== 1/4 train two seeds from scratch =="
for SEED in 3072 42; do
  CFG="configs/wcm_sparse_memory_scratch_s${SEED}.yaml"
  OUT="$BASE/wcm_sparse_memory_scratch_s${SEED}"
  if [[ -e "$OUT" ]]; then
    echo "[abort] output directory already exists; choose a fresh output_dir: $OUT"
    exit 1
  fi
  echo "[train] $CFG (no init_from)"
  "$PYTHON" -u -m world_critic.train --config "$CFG" \
    2>&1 | tee "outputs/batch_logs/wcm_sparse_memory_scratch_s${SEED}.log"
done
...
run_eval() {
  ...
  "$PYTHON" -u -m world_critic.evaluate \
    ...
    > "outputs/batch_logs/memory_scratch_${dataset_name}_${model_name}.log" 2>&1
}
...
for DATASET_NAME in 5cut ood; do
  ...
  for SEED in 3072 42; do
    ...
    run_eval "$DATASET_NAME" "$DATASET_ROOT" "memory_s$SEED" \
      "$BASE/wcm_sparse_memory_scratch_s$SEED/deploy.pt"
  done
done

echo "== 3/4 paired comparison: SparseMemory scratch vs Image-B4 =="
for DATASET_NAME in 5cut ood; do
  for SEED in 3072 42; do
    "$PYTHON" -m scripts.compare_experiments \
      ...
      --output "$EVAL_ROOT/comparisons/${DATASET_NAME}_memory_scratch_vs_image_b4_s${SEED}.json"
  done
done
