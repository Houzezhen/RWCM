"""One real alternating causal temporal optimizer step with CUDA memory reporting."""
...
def run() -> None:
    ...
    parser.add_argument("--config", default="configs/wcm_sparse_temporal_exp_s3072.yaml")
    ...
    frames = config.model.temporal_transformer_count
    ...
    loss.backward()
    gate_grad = sum(
        float(layer.residual_gate.grad.abs())
        for layer in model.temporal_adapter.layers
        if layer.residual_gate.grad is not None
    )
    if gate_grad <= 0:
        raise RuntimeError("Causal global residual gates received no gradient.")
    optimizer.step()
    ...
    result = {
        ...
        "global_blocks": len(model.temporal_adapter.layers),
        "global_gate_grad_sum": gate_grad,
    }
    if device.type == "cuda":
